...
- **[`src/graphs/`](src/graphs/mod.rs)** — wire each graph, auto-hash, and decode bytes to text for the text builders. Hashing is drft's job, done once per node at this seam — sources and builders never hash. Text normalization belongs in that decode, which builds an owned copy and never touches the walked files — so it cannot move a node's hash. Doing it in a source would, since a source's bytes are what gets hashed. The protection is the copy, not the order of the two steps.
...
- **[`src/rules/`](src/rules/check.rs)** — findings over the composed graph. `staleness.rs` joins the lockfile; `structural.rs` reads shape; `check.rs` applies config and severity, and owns the findings about the run itself rather than about a node — the ones that have to fire when there is no lockfile to join. It also receives findings raised during the build, either by a parser that recognized something it could not read or by the graph wiring when a matched file cannot reach a text parser. Both describe an absence the built graph cannot express.
...
1. Add the finding to `src/rules/staleness.rs` (lock-derived) or `src/rules/structural.rs` (shape-derived); return `Finding`s. A finding about the run rather than about a node — one that has to fire when there is no lockfile to join — belongs in `check.rs` instead, because `staleness.rs` only runs against a usable baseline. A finding only a parser can see — a block it recognized and could not read — is a `Diagnostic` on its `ParseResult`, which the builder turns into a `Finding`. A condition that disappears before a parser runs, such as invalid UTF-8 matched by a text graph, becomes a `Finding` at the graph-wiring seam. `check.rs` merges both before applying severity. Reach for either build-time route only when the built graph genuinely cannot express the condition.
