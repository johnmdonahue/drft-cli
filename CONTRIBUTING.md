...
- **[`src/rules/`](src/rules/check.rs)** — findings over the composed graph. `staleness.rs` joins the lockfile; `structural.rs` reads shape; `check.rs` applies config and severity, and owns the findings about the run itself rather than about a node — the ones that have to fire when there is no lockfile to join. It also receives findings raised during the build, by a parser that recognized something it could not read; those describe a file's absence from the graph, which the graph cannot express.
...
1. Add the finding to `src/rules/staleness.rs` (lock-derived) or `src/rules/structural.rs` (shape-derived); return `Finding`s. A finding about the run rather than about a node — one that has to fire when there is no lockfile to join — belongs in `check.rs` instead, because `staleness.rs` only runs against a usable baseline. A finding only a parser can see — a block it recognized and could not read — is a `Diagnostic` on its `ParseResult`, which the builder turns into a `Finding` and `check.rs` merges before severity is applied. Reach for that last route only when the built graph genuinely cannot express the condition.
