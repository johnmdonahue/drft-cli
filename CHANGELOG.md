...
- **`no-edge-keys` hint** (#137). A frontmatter graph declaring no `edge_keys` emits no edges. That is a supported shape — a graph may exist purely to seed node metadata, with no provenance edges at all — so it loads rather than failing, and the hint is what stops the other reading: a repo that believes it is tracking provenance and is not. `edge_keys = []` is that same state written out and behaves identically — an empty set names nowhere to look, so treating one as a mistake and the other as a shape would make the config say two things with one meaning. **This lifts an existing error:** `keys = []` used to be rejected as "the graph would track nothing", which was right only while a graph tracking nothing was assumed to be a mistake. `drft init`'s template now declares `edge_keys` rather than scaffolding a graph that tracks nothing.
