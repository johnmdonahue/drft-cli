...
- **Frontmatter line numbers survive a multi-line code span** (#119). The edge scan reads the block through a mask that blanks code spans, so a `path.md` written in prose is never mistaken for a link target — and the mask blanked a span's newlines along with everything else. A span crossing a line boundary shortened the masked block, so every entry below it resolved one line too high per newline swallowed: in `drft edges`, in `drft impact`, and in the location of every finding, which is where a reader navigates from. A real finding at a fabricated line costs more than its size suggests, because nothing indicates which part is wrong. The edge scan now reads a mask that keeps a span's newlines. Blanking them is separately load-bearing — fusing a span's lines is part of what lets some blocks parse at all — so the edge scan falls back to the old mask when the new one yields no parse, and the mask deciding whether a block _is_ frontmatter is untouched. No file gains or loses frontmatter, an edge, an anchor, or an exit code as a result; the only change is which line an occurrence reports.
