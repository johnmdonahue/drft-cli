...
- **A leading byte-order mark no longer costs a file its frontmatter** (#118). A frontmatter block opens with `---` at offset 0, so a BOM ahead of it meant no parser recognized the block. The file lost its metadata **and its declared edges**, and nothing said so: both parsers agreed there was no block, so no inconsistency surfaced and no finding fired — `detached-node` at exit 0, indistinguishable from a file that declared nothing. Some editors write a BOM by default, so this is silent data loss on ordinary files. The mark is now dropped in the text decode, which covers every text parser at once. It is **not** dropped from the bytes drft hashes: a file's `b3:` is still the blake3 of the file as it sits on disk, so no hash moves and no `stale-node` appears on files nobody edited. Recognizing the block also stops the markdown parser reading it as body text, where a single-paragraph block followed by its closing `---` was a setext heading and published an address the file does not answer to. **Upgrading surfaces `new-edge` on any BOM-carrying file that declares a source** — that is the edge appearing for the first time, not drift.
