...
- **A leading fenced block is frontmatter, and one set of fence rules decides where it ends** (#133). The markdown parser used to blank the block using a boundary the frontmatter parser derived, and the two disagreed on fifteen input classes. Where drft claimed a block the renderer did not, the same text became metadata _and_ a setext heading — `title: Doc` above a closing `----` published `#title-doc`, an address the document does not answer to, which `unresolved-fragment` then accepted from every link written to it. Where drft declined a block the renderer claimed, the block's text was read as body content. The rules are now the renderer's, mirrored rather than approximated: a block opens on exactly three dashes on a line of their own, its first line is neither blank nor a closing fence, and it closes on a line of exactly `---` or `...` followed only by spaces.
  ...
  **Three things change for an existing repository, and all three are visible on the first `check`.**
  ...
  A **mid-document** `---\nkey: value\n---` section is claimed wherever it sits, not only at the head of a file. Its headings and its body links leave the markdown graph, and the frontmatter parser does not read it because that parser is leading-only. A document with such a section loses those anchors and edges with no finding — the leading-block rule is what `unreadable-frontmatter` covers, and this case sits outside it.

  A block closed by `----` or `---x` **stops contributing metadata**, because neither is a closing fence. Those keys were being read out of what the renderer shows as prose, so the metadata was invented; a repo relying on it sees the keys disappear from `drft nodes`.

  A document opening with a `---` thematic break **above a blank line** is unaffected: a block's first line may not be blank, so its headings and links are untouched.
