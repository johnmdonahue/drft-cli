...
- **Project configuration and baselines live under `.drft/`** (#177). Move root-level `drft.toml` to `.drft/config.toml` and `drft.lock` to `.drft/lock.toml` without changing their contents. drft reads only the new paths. If it finds either legacy file, it exits 2 before parsing configuration, building the graph, recording usage, or writing output. A destination that already exists is a conflict; drft selects neither file and never overwrites one. `drft init` creates `.drft/config.toml` and performs no migration.
  ...
  The project directory remains the graph root, including when commands run from a descendant. Every `.drft` directory and its descendants are pruned from filesystem graphs, so configuration, baselines, temporary lock writes, and future project state cannot become nodes or lock entries. Repositories still choose whether to track either file through ordinary Git policy.
...
- **Wrong-base causes identify the matching occurrence lines** (#123). When an unresolved bare path would resolve from the graph root, its cause identifies the source lines carrying the selected spelling while preserving the aggregated finding and complete occurrence list.
...
Also removes the last place drft guessed. A frontmatter value used to become an edge only if it looked like a path, so a derivation written any other way vanished without a finding. The declared key now decides, and nothing asks what a value looks like.
...
- **`keys` is renamed `edge_keys`, and frontmatter edges no longer depend on what a value looks like** (#137, #134, #138, #112). The parser used to collect every string in the block and filter it through a shape heuristic — an explicit `./` prefix, a URI, or a plausible file extension, with anything containing a space rejected as prose. A value that failed produced no edge, no finding, and no record that the file had declared anything, so the commonest way to lose a derivation was to write one the heuristic did not recognize. Now the declared key is the whole of the signal: **every string value reachable through an `edge_keys` key is an edge**, and one naming nothing that resolves raises `unresolved-edge` exactly as a typo'd path does. The remedy is the reader's — fix the value, fix the config, or move the field to a key you did not declare. No alias for the old name, per no-backwards-compat.
  ...
  **If your `[graphs.frontmatter]` has no `keys` today, add `edge_keys` before upgrading, or the graph silently stops tracking anything.** Omitting the field used to mean "classify every value by its shape", which is what `drft init` scaffolded and what the docs recommended against but accepted. It now means "this graph has no edges", and the two are indistinguishable from the config alone. A repo that upgrades without migrating gets zero frontmatter edges at exit 0 — `drft impact` answers `no dependents found`, and with a tracked lockfile the loss shows as `removed-edge` findings until someone relocks. Name the keys your frontmatter actually uses:
  ...
  A config that _did_ declare `keys` fails loudly instead: the field is unknown, so drft exits 2 naming `edge_keys` as the accepted spelling. Only the shape-detection default degrades quietly, which is why it is called out here.

  Narrowing to named keys is itself a change in what the graph tracks: values under any other key stop being edges. That is the point of the field, but against a tracked lockfile each one leaves as a `removed-edge`, so review before relocking.
...
- **A leading fenced block is frontmatter, and the markdown library decides where it ends** (#133). drft used to decide that itself: `mask_frontmatter` took its boundary from the frontmatter parser, so both halves of drft agreed with each other and disagreed with CommonMark about which fences open and close a block. That ran in two directions, and both were defects.
  ...
  **What changes for an existing repository, all of it visible on the first `check`:**
  ...
  A leading block that is **not a YAML mapping** — a comment-only block, a bare title between fences, malformed YAML — is now claimed by its fences and read by nobody, so it loses the anchors it used to publish and any markdown link written inside it stops being an edge. It raises the new `unreadable-frontmatter` finding naming the file, so this is reported rather than silent — wherever a frontmatter graph covers the file, since that parser is what recognizes the block. Against a tracked lockfile the lost links arrive as `removed-edge`.

  A block is frontmatter **only at the head of a file**. The markdown library will claim a fenced block wherever one appears; drft takes its answer only at byte 0. So a `---\nkey: value\n---` section partway down a document, or one below a blank first line, stays prose — a thematic break, a setext heading, another thematic break — and keeps the headings and links a reader sees. Nothing here changes for such a file.

  A block closed by anything that is not exactly `---` or `...` — `----`, `---x`, `--- x`, `---\t` — **stops contributing metadata**, because none of them is a closing fence. Those keys were being read out of what the renderer shows as prose, so the metadata was invented. Where that text forms a setext heading the heading now appears in `anchors`; where it does not, the file simply carries less. A repo gating on `[rules.unresolved-edge] severity = "error"` can go from **failing to passing**.

  A block whose **first line is blank**, or whose first line is `...`, stops being frontmatter, because the renderer will not open one either. Its headings and links are untouched — that is the thematic-break case working — but a repo that was getting metadata and declared edges out of such a block **loses both, and no finding covers it**.

  **Those two classes are the silent ones.** In both, the fences form no block drft recognizes, so there is nothing for `unreadable-frontmatter` to report — that rule fires on a block that _is_ recognized and cannot be read. Reporting these would mean guessing that a thematic break was meant as frontmatter. Against a tracked lockfile both surface as `removed-edge`, so the silence is bounded to a repo with no baseline.

- **`drft lock` prints a result document** (#120, #125). Text reports `locked 2 nodes`, `locked 1 node, dropped 1 entry`, or `locked 0 nodes`, followed by the node names; JSON emits `{locked, dropped}`. A scoped lock names the nodes it wrote, which is how a resolution the caller did not expect becomes visible; `--all` reports the count alone, since it resolves nothing and the listing would be a copy of `drft.lock`. Both name what they dropped: `--all` reports the entries its rebuild removes, so widening an `ignore` pattern and relocking no longer takes files out of the baseline in silence. Naming one path twice counts once. It previously printed nothing at all, which is what made a lock covering nothing indistinguishable from one covering the files you meant. **This breaks any consumer asserting `drft lock` is silent.** In JSON it also moves `lock`'s hints from the stderr `{"hints": […]}` envelope into the result document, where every other command already carries them; in text they still follow the result on stderr, as they do for every command. No alias, per no-backwards-compat.

- **An unlocked node subsumes its outbound `new-edge` findings** (#120). Adding a new file that links two locked files previously reported one `new-edge` per link; it now reports one `unlocked-node` naming the file. The subsumption is applied after severity and ignore globs, so silencing `unlocked-node` restores those `new-edge` findings rather than dropping both, and a finding is only subsumed by one at least as severe as itself — a `warn` `unlocked-node` never swallows an `error` `new-edge`, which would turn a failing run into a passing one. The node having no baseline is the single fact behind every one of those edges, and this is the subsumption `stale-node` and `removed-node` already apply to their own edge findings. The trade is real: `new-edge` carried line numbers and the subsuming finding does not.
...
- **`unreadable-frontmatter` reports a block that was recognized and could not be read.** A leading fenced block is frontmatter by its fences alone, so its text is never rendered as content; when the YAML inside is not a mapping, the file contributes no metadata and no declared edges and used to look identical to a file that declares nothing. It is a rule rather than a hint because hints never change an exit code, and a repository whose derivations are declared in frontmatter needs a dropped block to be able to fail a run. Defaults to `warn`; `[rules] unreadable-frontmatter = "error"` makes it fail.
...
- **`nothing-to-lock` and `replaced-unreadable-lock` hints** (#125, #120). The first covers a locked path that carries no content to snapshot but is not a directory — an escaping symlink — which otherwise reported `locked 0 nodes` with no reason. The second fires when `drft lock --all` replaces a lockfile it could not read: the rebuild is correct, but it cannot know what the unreadable bytes held, so it says the drop list is incomplete rather than let an empty one read as "nothing was dropped".

- **A link naming the graph root resolves to `.`** rather than to the empty string. `..` from `docs/a.md` and `.` from a root file both name the graph root, which normalized away entirely: the edge target became `""`, a node reference the graph does not contain, and it was written to `drft.lock` and to the JGF export as a dangling one. The root carries no node — it is the graph rather than a member of it — so the edge is unresolved either way, but it is now a target you can read, and `..` (one level above the root) stays distinct from it. **A repo with such a link sees one `removed-edge` and one `new-edge` on the first check after upgrading**, and relocking settles it. This is a markdown-link fix as much as a frontmatter one.

- **Text output escapes control characters, so one record is always one line.** A path, a target, a node id, a frontmatter key, a graph name, or a hint locus can carry anything a filename or a YAML scalar can — including a newline, which put a second line into `drft check`, `nodes`, `edges`, `impact` and `lock` output carrying no severity or record prefix. Any log parser, grep, or hook reading text output read that as a different kind of record, and an unescaped key rendered one node as two blocks. Every text render site now escapes; **JSON, the JGF export and `drft.lock` are unchanged and carry the bytes the file carried**, which is what makes them the authoritative form. This is reachable on ordinary repositories only through an exotic filename, but a frontmatter value can hold a multi-line block scalar, and under this release every such value is an edge.
...
- **Repository `.gitignore` files above the graph root apply to discovery** (#111). A `drft.toml` in a repository subdirectory used to ignore only rules declared at or below that subdirectory, so a repository-root pattern could hide a file from Git while drft still indexed it. drft reads repository `.gitignore` files through the Git root while keeping `.ignore`, `.git/info/exclude`, and global excludes disabled. `drft config --show-ignores` reports the files and source classes used without building the graph or changing `drft.lock`.
...
- **A leading byte-order mark no longer costs a file its frontmatter** (#118). A frontmatter block opens with `---` at offset 0, so a BOM ahead of it meant no parser recognized the block. The file lost its metadata **and its declared edges**, and nothing said so: both parsers agreed there was no block, so no inconsistency surfaced and no finding fired — `detached-node` at exit 0, indistinguishable from a file that declared nothing. Some editors write a BOM by default, so this is silent data loss on ordinary files. Leading marks are now dropped in the text decode, which covers every text parser at once — marks plural, because a tool re-marking an already-marked file writes two, and stripping one would leave that file failing exactly as it did before. One consequence worth knowing: a U+FEFF that is genuinely content rather than a mark is indistinguishable at offset 0, so a file opening with one loses it from the text while keeping it in the hash. It is **not** dropped from the bytes drft hashes: a file's `b3:` is still the blake3 of the file as it sits on disk, so no hash moves and no `stale-node` appears on files nobody edited. Recognizing the block also stops the markdown parser reading it as body text, where a single-paragraph block followed by its closing `---` was a setext heading and published an address the file does not answer to. **Upgrading changes what `check` reports on mark-carrying files**, all of it the graph gaining what it should have had: `new-edge` where a file declares a source, `unresolved-edge` where the source it declares does not exist, `detached-node` disappearing as those files gain connections, and fragment findings moving as real anchors appear and fabricated ones vanish. None of it is drift. At default severities every rule involved is a warning and the exit code does not move. A repo that has raised one to `error` should know which: `new-edge` clears with the next `drft lock`, while `unresolved-edge` and `unresolved-fragment` are reporting a real problem the mark had been hiding, and stay until the file is fixed.

- **Frontmatter line numbers survive a multi-line code span** (#119). The edge scan reads the frontmatter block through a mask that blanks code spans, so that a span hiding a `:` cannot break the mapping. (The mask also gated a shape test at the time; that test is deleted later in this release, and the mask's remaining job is structural.) Blanking a span takes its newlines with it, which is deliberate — fusing a span's lines is part of what lets a block parse, since a span can hide a `:` that would otherwise break the mapping. It also made the masked copy shorter than the source, so every value below a span resolved one line too high per newline swallowed: a four-line span shifted by three. That wrong line is recorded on the edge and reaches `drft edges`, `drft impact`, and the location of every finding, which is where a reader navigates from. A real finding at a fabricated line costs more than its size suggests, because nothing indicates which part is wrong. The mask now records, for each masked line, which source lines its columns came from, and the edge scan corrects the reported line against that table. Per column rather than per line, because a value can sit after a multi-line span closes and share a masked line with the span's opening — mapping the whole line to where it began would report the opening line for a value the mask never touched. The masked text is unchanged, so the value a scalar carries — and therefore the edge target, the node it resolves to, and the lockfile entry — is byte-for-byte what it was; the reported line is the only thing that moves. (One derivative: `drft graph`'s large-projection hint states a byte size, which shifts when line numbers gain a digit.) A block that reaches frontmatter only through the fused mask now reports its real lines too, which it never did — and so does one containing a lone carriage return, which YAML counts as a line break where Rust's `str::lines` does not.

- **A code span in frontmatter no longer costs a block its edges** (#132). The edge scan read a copy of the block with code spans blanked, while the metadata read the raw block first. `strip_code` knows nothing about YAML, so a fence opened inside a block scalar latched its fenced pass and blanked every line below it — the declaring key included — and a backtick written as a literal character in two separate values blanked everything between them. The raw block stayed well-formed throughout, so drft contradicted itself: `drft nodes` printed the derivation, `drft edges` had none, and `detached-node` at exit 0 was the only thing said about the file. Nothing failed to parse, so no diagnostic keyed on a parse error can reach that class. The edge scan now chooses its rendering by the same test the metadata uses — the raw block where it is a YAML mapping, the masked copy otherwise — so the two cannot disagree about what a block says. Code spans were blanked before the link test at the time, per value rather than across the block, which kept a span from reaching past its own value. **Both the test and that per-value mask are deleted later in this release** — the declared key decides now — so a value carrying a span is an edge naming the value as written. **Upgrading moves findings on files carrying a code span in frontmatter, in three directions.** Edges appear where a block was losing them — the defect above. Edge targets change where a value contains a span: the target is the value as declared rather than the value with its spans blanked out, so `sources: ./notes.md` followed by a span names a target ending in that span and raises `unresolved-edge` instead of quietly resolving to `notes.md`. And more values became candidates than had been: one whose span blanks to surrounding whitespace, and — with no code span involved at all — one carrying leading or trailing whitespace, a tab, or the trailing newline a `|` block scalar keeps. Those were dropped in silence and mostly resolve, so a repo with no backticks anywhere in its frontmatter still gains edges here. (Candidacy itself is deleted later in this release; every string under a declared key is an edge.) At default severities the exit code does not move; a repo running `[rules] unresolved-edge = "error"` will see those runs fail, which is the config and the content disagreeing rather than drift.

- **A scoped lock no longer snapshots a file the caller did not name** (#127). A bare argument had `.md` appended and that invented candidate was tried _ahead_ of the path as spelled, so with both `docs/` and `docs.md` present, `drft lock docs` snapshotted `docs.md` — clearing its `stale-node` finding and writing a durable "this was reviewed" claim against a file nobody opened. The exact path now resolves first, in the lock resolver rather than the shared candidate helper — a reader only produces a projection, so the old order was harmless for `drft impact` and `drft nodes` and is unchanged there. The related case, where a bare name typed in a subdirectory falls through to a same-named node at the graph root, raises a new `resolved-elsewhere` hint naming the node actually locked. Reporting the locked key alone could not surface it, because that key is byte-identical to what the caller typed.

- **A directory lock no longer manufactures an empty baseline** (#125). In a repo that had never been locked, `drft lock <dir>` wrote a valid, parseable, zero-entry lockfile — a baseline covering nothing, produced by a command that reported success, which made every staleness rule a no-op while the file's presence made the baseline look established. A lock that writes nothing now leaves no lockfile behind. `drft lock --all` still rewrites an existing lockfile whatever it finds, including down to no entries — it is the rebuild, so afterwards the file reflects the tree, and skipping that write would leave entries for deleted files standing as `removed-node` findings the rebuild could not clear.
