...
- **A leading fenced block is frontmatter, and the markdown library decides where it ends** (#133). drft used to decide that itself: `mask_frontmatter` took its boundary from the frontmatter parser, so both halves of drft agreed with each other and disagreed with CommonMark on fifteen of twenty-four measured shapes. That ran in two directions, and both were defects.
  ...
  Where **the renderer claims a block drft does not** — a setext title, a comment-only block, a `---` opener, a `...` closer — nothing masked it, so the markdown parser read the block's YAML as body content and slugged anchors out of it. That is the fabricated address of #133, and it silently accepted every link written to it.

  The rules are now the library's, mirrored in the frontmatter parser rather than approximated: a block opens on a line of exactly three dashes carrying nothing but whitespace, its first line is neither blank nor a closing fence, and it closes on a line of exactly `---` or `...` followed only by spaces. **The mirror covers fence syntax, not position** — see the mid-document note below.

  **Three things change for an existing repository, and all three are visible on the first `check`.**

  A leading block that is **not a YAML mapping** — a comment-only block, a bare title between fences, malformed YAML — is now claimed by its fences and read by nobody, so it loses the anchors it used to publish and any markdown link written inside it stops being an edge. It raises the new `unreadable-frontmatter` finding naming the file, so this is reported rather than silent. Against a tracked lockfile the lost links arrive as `removed-edge`.

  A **non-leading** `---\nkey: value\n---` section is claimed wherever it sits. Its headings and its body links leave the markdown graph, and the frontmatter parser reads only a block at the very start of a file, so nothing reads it. A document with such a section loses those anchors and edges **with no finding** — the new rule covers a leading block that cannot be read, not a block nobody reads. This reaches more than mid-document sections: a file opening with `---\n---`, with `---` above a blank line, or with `----`, has its _first_ block declined and a _later_ one claimed, and a file carrying two blocks has only the first read.

  A block closed by `----`, `---x` or `---\t` **stops contributing metadata**, because none of them is a closing fence. Those keys were being read out of what the renderer shows as prose, so the metadata was invented; a repo relying on it sees the keys disappear from `drft nodes`, and the heading that text forms appears in `anchors` where it belongs. A repo gating on `[rules.unresolved-edge] severity = "error"` can go from **failing to passing** here, because a declared edge that was only ever read out of prose stops existing.

  A block whose **first line is blank**, or whose first line is `...`, stops being frontmatter, because the renderer will not open one either. Its headings and body links are untouched — that is the thematic-break case working — but a repo that was getting metadata and declared edges out of such a block **loses both, and no finding covers it**: nothing was recognized, so there is no unreadable block to report. Against a tracked lockfile the lost derivations arrive as `removed-edge`. This is the one silent direction in this release, and it is silent on purpose — reporting it would mean guessing that a thematic break was meant as frontmatter.
