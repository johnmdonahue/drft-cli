...
- **A leading fenced block is frontmatter, and the markdown library decides where it ends** (#133). drft used to decide that itself: `mask_frontmatter` took its boundary from the frontmatter parser, so both halves of drft agreed with each other and disagreed with CommonMark about which fences open and close a block. That ran in two directions, and both were defects.
  ...
  **Three things change for an existing repository, and all three are visible on the first `check`.**
  ...
  A block closed by `----`, `---x` or `---\t` **stops contributing metadata**, because none of them is a closing fence. Those keys were being read out of what the renderer shows as prose, so the metadata was invented. Where that text forms a setext heading it now appears in `anchors`, where it belongs; where it does not, the file simply carries less. A repo gating on `[rules.unresolved-edge] severity = "error"` can go from **failing to passing**.
