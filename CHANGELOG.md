...
And it holds frontmatter to the same standard: nothing a file declares is discarded or misreported without saying so.
...
- **Frontmatter line numbers survive a multi-line code span** (#119). The parser masks code spans before scanning a block for edges, and the mask replaced a span's newlines with spaces along with everything else. A span crossing a line boundary therefore shortened the masked block by a line, and every entry below it reported one line too high — in `drft edges`, in `drft impact`, and in the location of every finding, which is where a reader navigates from. The mask now keeps a span's newlines and blanks the rest. One visible consequence: collapsing those newlines could fuse two source lines into a mapping entry that appears nowhere in the file, so anything reading `@frontmatter` on such a file was reading invented keys. A block that parsed only because of the collapse now yields no `@frontmatter` metadata at all, which is the correct outcome — it was never a mapping.
